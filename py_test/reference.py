...
def compute_merl_tab_ndf_reference(input_map, cubemap_res, ref_res, merl_ndf, save_file_name = None):
    """
    Numerical filter the image
    :param input_map:
    :param cubemap_res:
    :param ref_res:
    :param merl_ndf:
        ...
        for face_idx in range(6):
            for i in range(ref_res):
                for j in range(ref_res):
                    ...
                    # The computation of cosine requires all normalized vectors
                    # integral = loop_compute(normal_direction,normalized_xyz_input_tmp,j_weighted_input_tmp,GGX_alpha,delta_s)
                    integral = loop_compute_merl_ndf_nojit(normal_direction, normalized_xyz_input_tmp, j_weighted_input_tmp,
                                                  merl_ndf,
                                                  delta_s)
                    # cosine = np.dot(normalized_xyz_input,normal_direction)
                    # ndf = GGX.ndf_isotropic(cosine)
